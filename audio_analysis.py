...
mfccs   = mfcc(y,               sr=sr, hop_length=hop_length, window='hann', n_fft=win_length)
mels    = melspectrogram(y=y,   sr=sr, hop_length=hop_length, window='hann', n_fft=win_length)
jumps    = np.sum(np.diff(mfccs.T, axis=0)**2, axis=1)
...
## Clustering  
n_clusters = 3 ##3 or 4 seems decent
...
def init():
    ...
    line1, = librosa.display.specshow(power_to_db(mels,ref=np.max), x_coords=time[::hop_length], y_axis='mel', fmax=fmax, ax=ax1)
    line2, = ax2.plot(time[::hop_length], km[1])
    line4, = ax3.plot(time[hop_length::hop_length], jumps)
...
def animate(i):
    ...
    line4, = ax3.plot(time[hop_length::hop_length][1:], jumps[1:])
    ...
    return [line2, line3, line4, title]
